import { BellIcon } from "@heroicons/react/24/outline";
import Image from "next/image";
import { DatePickerDemo } from "./DateComponents";
import { DropdownMenuCheckboxes } from "./ProfileDropdown";

const Header = () => {
  return (
    <div className="relative hidden  ml-20 lg:flex justify-between items-center bg-[#FAFAFA] dark:bg-[var(--background)] p-4">
      <h1 className=" font-bold capitalize text-[#26282C] dark:text-white text-xl">
        Dashboard
      </h1>

      <div className="flex items-center gap-x-6">
        <div className=" relative py-4 px-5 rounded-full w-80 bg-white dark:bg-[var(--background)]  ">
          <svg
            width="18"
            height="18"
            viewBox="0 0 18 18"
            className=" absolute top-1/3 left-2.5 cursor-pointer "
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              fill-rule="evenodd"
              clip-rule="evenodd"
              d="M8.68945 1C12.9293 1 16.3781 4.3727 16.3781 8.51907C16.3781 10.4753 15.6104 12.2595 14.3542 13.5986L16.8261 16.0109C17.0574 16.2371 17.0582 16.6031 16.8269 16.8294C16.7116 16.9436 16.5592 17 16.4076 17C16.2568 17 16.1052 16.9436 15.9892 16.8309L13.4874 14.3912C12.1714 15.4219 10.5028 16.0389 8.68945 16.0389C4.44955 16.0389 1 12.6655 1 8.51907C1 4.3727 4.44955 1 8.68945 1ZM8.68945 2.15821C5.10251 2.15821 2.18433 5.01125 2.18433 8.51907C2.18433 12.0269 5.10251 14.8807 8.68945 14.8807C12.2756 14.8807 15.1938 12.0269 15.1938 8.51907C15.1938 5.01125 12.2756 2.15821 8.68945 2.15821Z"
              fill="#78828A"
            />
          </svg>
          <input
            type="text"
            placeholder="Search..."
            className="px-4 focus:outline-none bg-white dark:bg-[var(--background)]  text-black dark:text-white"
          />
        </div>

        <div className="flex items-center gap-6 text-sm">
          <div className="flex items-center">
            <span className="">
              <DatePickerDemo />
            </span>
          </div>
          <div className=" border border-[#DADDDD] p-1 rounded-full">
            <BellIcon className="h-6 w-6" />
          </div>
        </div>

        <DropdownMenuCheckboxes />
      </div>
    </div>
  );
};

export default Header;
